      ...
      if (File.Exists(fileName))
      {
        ...
        for (int y = 0; y < heightMap.Count; y++)
        {
          for (int x = 0; x < heightMap[y].Count; x++)
          {
            ...
            if ((y == 0 || x == 0) && heightMap[y][x] == 'a')
            {
              startLocations.Add(new int[] { x, y });
            }
          }
        }

        // Calculate shortest path for all possible start locations
        int shortestPath = int.MaxValue;
        foreach (int[] startLocation in startLocations)
        {
          int currentShortestPath = findShortestPath(heightMap, startLocation[0], startLocation[1], endLocation[0], endLocation[1]);
          if (currentShortestPath > 0 && currentShortestPath < shortestPath)
          {
