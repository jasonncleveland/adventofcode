...
/*
 * To compile: dotnet build part1/build.csproj
 * To run: dotnet run --project part1/build.csproj -- problemData.txt
 */
class ClumsyCrucible
{
  ...
  static int traverseGrid(List<List<int>> grid, int startRow, int startColumn, int targetRow, int targetColumn)
  {
    ...
    while (directionsToCheck.Count > 0)
    {
      ...
      // We can only travel at most 3 spaces before needing to change direction
      ...
      List<GridCoordinate> neighbours = getNeighbours(row, column, direction);
  ...
  static List<GridCoordinate> getNeighbours(int row, int column, Direction direction)
  {
