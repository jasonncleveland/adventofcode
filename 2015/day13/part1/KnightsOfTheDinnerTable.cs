      ...
      if (File.Exists(fileName))
      {
        ...
        foreach (var permutation in permutations)
        {
          ...
          for (int i = 0; i < permutation.Count; i++)
          {
            ...
            {
              // left
              totalHappiness += permutation[i].HappinessUnits[permutation[permutation.Count - 1].Name];
              // right
              totalHappiness += permutation[i].HappinessUnits[permutation[i + 1].Name];
            }
            else if (i == permutation.Count - 1)
            {
              ...
              totalHappiness += permutation[i].HappinessUnits[permutation[i - 1].Name];
              // right
              totalHappiness += permutation[i].HappinessUnits[permutation[0].Name];
            }
            else
            {
              // left
