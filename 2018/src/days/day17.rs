                ...
                match left.0 {
                    "x" => {
                        if let Ok(x) = left.1.parse::<i64>()
                            && let Some((Ok(y1), Ok(y2))) = right
                                .1
                                .split_once("..")
                                .map(|s| (s.0.parse::<i64>(), s.1.parse::<i64>()))
                        {
                            for y in y1..=y2 {
                                if y < min_y {
                                    min_y = y;
        ...
        None => {
            ...
            match map.get(&down) {
                ...
                None => {
                    ...
                    let is_settled =
                        flood_vertical(map, down, &Direction::Down, min_y, max_y, visited);
                    if is_settled {
