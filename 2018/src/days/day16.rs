...
pub fn solve(file_contents: String) -> (String, String) {
    ...
    let part1 = solve_part_1(&samples);
    debug!("Part 1: {} ({:?})", part1, part1_timer.elapsed());

    let part2_timer = Instant::now();
    let part2 = solve_part_2(&samples, &instructions);
...
fn parse_input(file_contents: String) -> (Vec<Sample>, Vec<Instruction>) {
    ...
    if let Some((left, right)) = file_contents.split_once("\n\n\n\n") {
        // Parse samples
...
fn solve_part_1(samples: &Vec<Sample>) -> i64 {
...
fn solve_part_2(samples: &Vec<Sample>, instructions: &Vec<Instruction>) -> usize {
    ...
    for sample in samples {
        ...
        if match_count == 1 && let Some(name) = possible_opcodes.first() {
