...
internal sealed class Day08 : AbstractDaySolver<IReadOnlyList<(string, int)>>
{
    protected override IReadOnlyList<(string, int)> ParseInput(ILogger logger, string fileContents)
    {
    ...
    protected override string SolvePart1(ILogger logger, IReadOnlyList<(string, int)> operations)
    {
        return ExecuteProgram(operations).ToString();
    }

    protected override string SolvePart2(ILogger logger, IReadOnlyList<(string, int)> operations)
    {
        List<(string operation, int offset)> fixedOperations = new(operations);
        ...
        for (int instructionPointer = 0; instructionPointer < operations.Count; instructionPointer++)
        {
            ...
            mapping.TryAdd(nextInstructionPointer, []);
            flippedMapping.TryAdd(flippedNextInstructionPointer, []);
        ...
        bool success = FindOperationToFlipRec(mapping, flippedMapping, reverseInstructionPointer, checkedInstructions,
            out (string operation, int instructionPointer) instructionToFlip);

        // Change the correct instruction
        fixedOperations[instructionToFlip.instructionPointer] = (instructionToFlip.operation, operations[instructionToFlip.instructionPointer].Item2);

        // Run the instructions with the correct operation flipped
        return ExecuteProgram(fixedOperations, true).ToString();
    }

    private static long ExecuteProgram(IReadOnlyList<(string, int)> operations, bool loopError = false)
    {
        long accumulator = 0;
     ...
     */
    private static bool FindOperationToFlipRec(Dictionary<int, List<(string, int)>> mapping,
        Dictionary<int, List<(string, int)>> flippedMapping, int instructionPointer, HashSet<int> checkedInstructions,
        out (string, int) instructionToFlip, int flippedOperations = 0)
    {
        ...
        if (mapping.TryGetValue(instructionPointer, out List<(string, int)> normalOperation))
        {
            foreach ((string operation, int nextInstructionPointer) instruction in normalOperation)
            {
                ...
                bool success = FindOperationToFlipRec(mapping, flippedMapping, instruction.nextInstructionPointer,
                    checkedInstructionsCopy, out instructionToFlip, flippedOperations);
                if (success)
                {
                    return true;
                }
        ...
        if (flippedOperations < 1 && flippedMapping.TryGetValue(instructionPointer, out List<(string, int)> flippedOperation))
        {
            foreach ((string operation, int nextInstructionPointer) instruction in flippedOperation)
            {
                if (checkedInstructions.Contains(instruction.nextInstructionPointer))
                {
                    // Stop if we find an infinite loop
                ...
                checkedInstructionsCopy.Add(instruction.nextInstructionPointer);
                bool success = FindOperationToFlipRec(mapping, flippedMapping, instruction.nextInstructionPointer,
                    checkedInstructionsCopy, out instructionToFlip, flippedOperations + 1);
                if (success)
                {
