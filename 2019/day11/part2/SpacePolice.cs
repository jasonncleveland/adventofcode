    ...
    if (args.Length > 0)
    {
      ...
      if (File.Exists(fileName))
      {
        ...
        foreach (string line in lines)
        {
          ...
          // Start painting at 0,0 on a white panel facing up
          int currentX = 0;
          int currentY = 0;

          Direction currentDirection = Direction.UP;
          computer.InputValues.Enqueue(1);
          ...
          while (true)
          {
            ...
            catch (NoInputValues)
            {
              ...
              {
                Console.WriteLine($"bad coordinates: {currentY},{currentX}");
              }
              ...
              // Check and update the x and y boundaries if needed
            ...
            catch (EndOfProgram)
            {
              ...
              Console.WriteLine($"Finished at {currentY},{currentX} facing direction {currentDirection}");
              break;
            }
          }
          ...
          Console.WriteLine($"X range: [{minX},{maxX}] Y range: [{minY},{maxY}]");

          // Create and populate a grid to store the paint information
          // This operates on the assumption that the coordinates are positive
          ...
          for (int y = 0; y < grid.Length; y++)
          {
            ...
            {
              if (paintedSquares.ContainsKey($"{y},{x}"))
              {
                grid[y][x] = paintedSquares[$"{y},{x}"] == 1 ? '#' : '.';
              }
              else
              {
                grid[y][x] = '.';
